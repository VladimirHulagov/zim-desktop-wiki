...
"""),  # T: plugin description
    ...
    plugin_preferences = (
        ...
        (
            "use_drag_and_drop",
            "bool",
            _("Use drag&drop to move pages in the notebook"),
            False,
        ),
        # T: preferences option
        ...
        (
            "autocollapse",
            "bool",
            _("Automatically collapse sections on close page"),
            True,
        ),
        # T: preferences option
        (
            "use_hscroll",
            "bool",
            _("Use horizontal scrollbar (may need restart)"),
            False,
        ),
        # T: preferences option
        ...
        (
            "sort_order",
            "choice",
            _("Sort pages by"),  # T: preferences option
            SORT_BY_NAME,
            ((SORT_BY_NAME, _("Name")), (SORT_BY_MTIME, _("Modification time"))),
        ),
    )
...
class PageIndexNotebookViewExtension(NotebookViewExtension):
    def __init__(self, plugin, pageview):
        ...
        sort_by = plugin.preferences.get("sort_order", SORT_BY_NAME)
        model = PageTreeStore(index, sort_by=sort_by)
        ...
        self.treeview.connect("populate-popup", self._on_populate_popup)

    def _on_populate_popup(self, treeview, menu):
        sep = Gtk.SeparatorMenuItem()
        menu.append(sep)
        sort_item = Gtk.CheckMenuItem(_("Sort by modification time"))
        # T: context menu option in page index
        current = self.plugin.preferences.get("sort_order", SORT_BY_NAME)
        sort_item.set_active(current == SORT_BY_MTIME)
        sort_item.connect("toggled", self._on_sort_toggled)
        menu.append(sort_item)
        menu.show_all()

    def _on_sort_toggled(self, menuitem):
        if menuitem.get_active():
            self.plugin.preferences["sort_order"] = SORT_BY_MTIME
        else:
            self.plugin.preferences["sort_order"] = SORT_BY_NAME

    def on_preferences_changed(self, preferences):
        # Check if sort order changed — requires model rebuild
        new_sort = preferences.get("sort_order", SORT_BY_NAME)
        current_model = self.treeview.get_model()
        if hasattr(current_model, "_sort_by") and current_model._sort_by != new_sort:
            self.treeview.disconnect_index()
            index = self.pageview.notebook.index
            model = PageTreeStore(index, sort_by=new_sort)
            self.treeview.set_model(model)
            self.on_page_changed(self.pageview, self.pageview.page)

        # Disable drag-and-drop when sorting by mtime (manual order makes no sense)
        if new_sort == SORT_BY_MTIME:
            self.treeview.set_use_drag_and_drop(False)
        else:
        ...
        self.treeview.set_autoexpand(
            preferences["autoexpand"], preferences["autocollapse"]
        )
    ...
    def reload_model(self):
        ...
        """
        self.treeview.disconnect_index()
        sort_by = self.plugin.preferences.get("sort_order", SORT_BY_NAME)
        model = PageTreeStore(self.pageview.notebook.index, sort_by=sort_by)
        self.treeview.set_model(model)

    @action(
        _("Collapse Page Index"), accelerator=None, menuhints="view"
    )  # T: menu item
    def collapse(self):
        self.treeview.collapse_all()

    @action(
        _("Reveal Current Page in Page Index"), accelerator=None, menuhints="view"
    )  # T: menu item
    def reveal(self):
...
class PageTreeStore(PagesTreeModelMixin, PageTreeStoreBase):
    def __init__(self, index, root=None, reverse=False, sort_by=SORT_BY_NAME):
        PagesTreeModelMixin.__init__(self, index, root, reverse, sort_by)
...
class PageTreeView(BrowserTreeView):
    ...
    def __init__(self, notebook, navigation, model=None):
        ...
        column.set_attributes(
            cr1, text=NAME_COL, style=STYLE_COL, sensitive=EXISTS_COL, weight=WEIGHT_COL
        )
    ...
    def set_use_drag_and_drop(self, use_drag_and_drop):
        if use_drag_and_drop:
            ...
            self.enable_model_drag_dest((PAGELIST_TARGET,), Gdk.DragAction.MOVE)
        else:
    ...
    def do_key_press_event(self, event):
        ...
        return handled or BrowserTreeView.do_key_press_event(self, event)
