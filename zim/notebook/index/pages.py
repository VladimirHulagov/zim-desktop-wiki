...
from zim.notebook.layout import FILE_TYPE_PAGE_SOURCE, FILE_TYPE_ATTACHMENT

ROOT_PATH = Path(":")
ROOT_ID = 1  # Constant for the ID of the root namespace in "pages"
# (Primary key starts count at 1 and first entry will be root)

PAGE_EXISTS_UNCERTAIN = (
    0  # e.g. folder with unknown children - not shown to outside world
)
...
class PagesIndexer(IndexerBase):
    ...
    def __init__(self, db, layout, filesindexer):
        ...
        self.layout = layout
        self.connectto_all(
            filesindexer, ("file-row-inserted", "file-row-changed", "file-row-deleted")
        )
        ...
        self.db.executescript("""
			...
			CREATE INDEX IF NOT EXISTS pages_mtime ON pages(parent, mtime);
		""")
    ...
    def _insert_page(self, pagename, is_link_placeholder, file_id=None):
        ...
        try:
            ...
            self.db.execute(
                "UPDATE pages SET sortkey=? WHERE name=?", (sortkey, pagename.name)
            )
            row = self._select(pagename)
        else:
            row = self._select(pagename)
    ...
    def update_parent(self, parentname, allow_cleanup=lambda r: True, oldrow=None):
        ...
        if n_children == 0 and row["source_file"] is None and allow_cleanup(row):
            ...
            self.db.execute(
                "UPDATE pages SET n_children=? WHERE id=?", (n_children, row["id"])
            )
    ...
    def update_page(self, pagename, mtime, content):
        ...
        self.emit("page-row-changed", row, oldrow)
    ...
    def _update_parent_nchildren(self, parentname):
        ...
        (n_children,) = self.db.execute(
            "SELECT count(*) FROM pages WHERE parent=?", (row["id"],)
        ).fetchone()
        self.db.execute(
            "UPDATE pages SET n_children=? WHERE id=?", (n_children, row["id"])
        )
...
class PageIndexRecord(Path):
    ...
    @property
    def hascontent(self):
        return self._row["source_file"] is not None

    @property
    def haschildren(self):
        return self._row["n_children"] > 0

    @property
    def n_children(self):
        return self._row["n_children"]

    @property
    def mtime(self):
        return self._row["mtime"]
...
class PagesViewInternal(object):
    ...
    def resolve_link(self, source, href, ignore_link_placeholders=True, source_id=None):
        parent, parent_id, names = self._resolve_link(
            source, href, ignore_link_placeholders, source_id
        )
        return self.resolve_pagename(parent, names)

    def _resolve_link(
        self, source, href, ignore_link_placeholders=True, source_id=None
    ):
        ...
        if href.rel == HREF_REL_RELATIVE:
            ...
            if ignore_link_placeholders:
                ...
                c = self.db.execute(
                    "SELECT name, id FROM pages WHERE sortkey=? ORDER BY name DESC",
                    (anchor_key,),
                )  # sort longest first
    ...
    def resolve_pagename(self, parent, names, parent_id=None):
        ...
        for i, basename in enumerate(names):
            ...
            candidates = self.db.execute(
                "SELECT id, name FROM pages WHERE parent=? and sortkey=? ORDER BY name",
                (page_id, sortkey),
            ).fetchall()
    ...
    def walk(self, parent_id):
        ...
        for row in self.db.execute(
            "SELECT * FROM pages WHERE parent=? ORDER BY sortkey, name", (parent_id,)
        ):
...
class PagesView(IndexView):
    ...
    ) -> Generator[PageIndexRecord, None, None]:
    ...
    def _list_pages(self, page_id: int):
        for row in self.db.execute(
            "SELECT * FROM pages WHERE parent=? ORDER BY sortkey, name", (page_id,)
        ):
            yield PageIndexRecord(row)
    ...
    def match_pages(
        self, path: Path, text: str, limit: int = 10
    ) -> Generator[PageIndexRecord, None, None]:
    ...
    def match_all_pages_by_words(
        self, words: list, limit: int = 10
    ) -> Generator[PageIndexRecord, None, None]:
    ...
    def get_has_previous_has_next(self, path: Path) -> bool:
        ...
        r = self.db.execute(
            "SELECT * FROM pages WHERE parent=? ORDER BY sortkey ASC, name ASC LIMIT 1",
            (ROOT_ID,),
        ).fetchone()
    ...
    def get_next(self, path: Path) -> Optional[Path]:
        ...
        r = self.db.execute("SELECT * FROM pages WHERE name=?", (path.name,)).fetchone()
        if r is None:
            raise IndexNotFoundError("No such page: %s" % path)

        if r["n_children"] > 0:
            r = self.db.execute(
                "SELECT name FROM pages WHERE parent=? ORDER BY sortkey, name LIMIT 1",
                (r["id"],),
            ).fetchone()
            if r is None:
                raise IndexConsistencyError("Missing children")
...
SORT_BY_NAME = "name"  #: Sort pages by name (natural sort) - default
SORT_BY_MTIME = "mtime"  #: Sort pages by modification time (newest first)
...
class PagesTreeModelMixin(TreeModelMixinBase):
    ...
    def __init__(self, index, root=None, reverse=False, sort_by=SORT_BY_NAME):
    ...
    def _order_clause(self):
        """Returns SQL ORDER BY clause based on current sort settings"""
        if self._sort_by == SORT_BY_MTIME:
            if self._REVERSE:
                return "ORDER BY mtime ASC, name ASC"
            else:
                return "ORDER BY mtime DESC, name DESC"
        else:
            if self._REVERSE:
                return "ORDER BY sortkey DESC, name DESC"
            else:
                return "ORDER BY sortkey ASC, name ASC"
    ...
    def on_page_row_changed(self, o, row, oldrow):
        if (
            self._sort_by == SORT_BY_MTIME
            and oldrow is not None
            and row["mtime"] != oldrow["mtime"]
        ):
            # mtime changed and we sort by mtime — position may have changed.
            # Compute old treepath (using old mtime) and new treepath.
            old_treepaths = self._find_treepaths_for_mtime(row["name"], oldrow["mtime"])
            self.flush_cache()
            new_treepaths = self._find_all_pages(row["name"])

            for old_tp in old_treepaths:
                self.emit("row-deleted", old_tp)

            for new_tp in new_treepaths:
                treeiter = self.get_iter(new_tp)  # not mytreeiter !
                self.cache[tuple(new_tp)].row = row
                self.emit("row-inserted", new_tp, treeiter)
                # If page has children, notify GTK so expander is shown
                if row["n_children"] > 0:
                    self.emit("row-has-child-toggled", new_tp, treeiter)
        else:
            # update row in cache
            for treepath in self._find_all_pages(row["name"]):
                treeiter = self.get_iter(treepath)  # not mytreeiter !
    ...
    def get_mytreeiter(self, treepath):
        ...
        if not parentpath:
            ...
            parent_iter = self.cache.get(parentpath, None) or self.get_mytreeiter(
                parentpath
            )  # recurs
            if parent_iter:
        ...
        for i, row in enumerate(rows):
            ...
            if mytreepath not in self.cache:
                self.cache[mytreepath] = MyTreeIter(
                    Gtk.TreePath(mytreepath), row, row["n_children"], IS_PAGE
                )
            else:
    ...
    def _find_treepaths_for_mtime(self, name, old_mtime):
        """Compute the treepath a page would have had with a given mtime.

        This is used when a page's mtime changes while sorting by mtime:
        the DB already holds the new mtime, so we need to reconstruct
        where the page *was* before the update by counting siblings using
        the old mtime value.

        Only the leaf level of the page path is affected; ancestor
        positions are read from the current DB state (their mtime did
        not change).
        """
        if self._MY_ROOT_ID is None or not name.startswith(self._MY_ROOT_NAME_C):
            return []
        ...
        treepath = []
        for i, basename in enumerate(names):
            page_name = self._MY_ROOT_NAME_C + ":".join(names[: i + 1])
            myrow = self.db.execute(
                "SELECT * FROM pages WHERE name=?", (page_name,)
            ).fetchone()
            if myrow is None:
                return []

            is_leaf = i == len(names) - 1
            if is_leaf and self._sort_by == SORT_BY_MTIME:
                # Use old_mtime to compute the position this page had
                # *before* the DB was updated.  Other siblings still
                # carry their unchanged mtime, but this page's row in
                # the DB already has the new mtime.  We therefore
                # exclude the page itself from the count and then add
                # a manual comparison with old_mtime.
                mtime = old_mtime
                if self._REVERSE:
                    # ORDER BY mtime ASC, name ASC
                    if mtime is None:
                        row = self.db.execute(
                            "SELECT COUNT(*) FROM pages "
                            "WHERE parent=? AND id!=? AND mtime IS NULL AND name<?",
                            (parent_id, myrow["id"], page_name),
                        ).fetchone()
                    else:
                        row = self.db.execute(
                            "SELECT COUNT(*) FROM pages "
                            "WHERE parent=? AND id!=? AND ("
                            "  mtime IS NULL OR mtime<? OR (mtime=? AND name<?)"
                            ")",
                            (parent_id, myrow["id"], mtime, mtime, page_name),
                        ).fetchone()
                else:
                    # ORDER BY mtime DESC, name DESC
                    if mtime is None:
                        row = self.db.execute(
                            "SELECT COUNT(*) FROM pages "
                            "WHERE parent=? AND id!=? AND ("
                            "  mtime IS NOT NULL OR (mtime IS NULL AND name>?)"
                            ")",
                            (parent_id, myrow["id"], page_name),
                        ).fetchone()
                    else:
                        row = self.db.execute(
                            "SELECT COUNT(*) FROM pages "
                            "WHERE parent=? AND id!=? AND ("
                            "  mtime>? OR (mtime=? AND name>?)"
                            ")",
                            (parent_id, myrow["id"], mtime, mtime, page_name),
                        ).fetchone()
            else:
                # For ancestor levels, use the normal computation —
                # their mtime has not changed so the current DB state
                ...
                if self._sort_by == SORT_BY_MTIME:
                    if self._REVERSE:
                        if mtime is None:
                            row = self.db.execute(
                                "SELECT COUNT(*) FROM pages "
                                "WHERE parent=? AND mtime IS NULL AND name<?",
                                (parent_id, page_name),
                            ).fetchone()
                        else:
                            row = self.db.execute(
                                ...
                                "  mtime IS NULL OR mtime<? OR (mtime=? AND name<?)"
                                ")",
                                (parent_id, mtime, mtime, page_name),
                            ).fetchone()
                    else:
                        if mtime is None:
                            row = self.db.execute(
                    ...
                    if self._REVERSE:
                        row = self.db.execute(
                            ...
                            "  sortkey>? OR (sortkey=? AND name>?)"
                            ")",
                            (parent_id, sortkey, sortkey, page_name),
                        ).fetchone()
                    else:
                        row = self.db.execute(
                            "SELECT COUNT(*) FROM pages "
    ...
    def _find_all_pages(self, name, update_cache=True):
        ...
        for i, basename in enumerate(names):
            ...
            if update_cache:
                ...
                if mytreepath not in self.cache:
                    myiter = MyTreeIter(
                        Gtk.TreePath(mytreepath), myrow, myrow["n_children"], IS_PAGE
                    )
    ...
    def assertPagesDBConsistent(self, db):
        for row in db.execute("SELECT * FROM pages"):
            ...
            self.assertEqual(
                row["n_children"],
                count,
                'Count for "%s" is %i while n_children=%i'
                % (row["name"], row["n_children"], count),
            )

            if row["source_file"] is not None:
                self.assertFalse(
                    row["is_link_placeholder"],
                    "Placeholder status for %s is wrong (has source itself)"
                    % row["name"],
                )
                ...
                self.assertIsNotNone(
                    child, "Missing child with source for %s" % row["name"]
                )
            ...
            if row["id"] > 1:
                ...
                self.assertIsNotNone(parent, "Missing parent for %s" % row["name"])

                if not row["is_link_placeholder"]:
                    # Check upwards - parent(s) must not be placeholder either
                    self.assertFalse(
                        parent["is_link_placeholder"],
                        "Placeholder status for parent of %s is inconsistent"
                        % row["name"],
                    )
